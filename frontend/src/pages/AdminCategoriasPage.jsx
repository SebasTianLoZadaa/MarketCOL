...
const AdminCategoriasPage = () => {
  ...
  const totalPaginas = Math.ceil(
    categoriasFiltradas.length / registrosPorPagina
  );
  ...
  const handleToggleActivo = async categoria => {
    try {
      ...
      setMensaje({
        tipo: 'success',
        texto: `Categoría ${
          !categoria.activo ? 'activada' : 'desactivada'
        } exitosamente`
      });
          ...
          <Row className="mt-2">
            <Col>
              <small className="text-muted">
                Mostrando {categoriasFiltradas.length} de{' '}

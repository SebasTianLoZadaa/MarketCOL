...
type ProductosPedidoProps = {
    detalles?: Detalle[];
    total?: number;
};

type AccionesPedidoProps = {
    estado?: string;
            ...
            {detalles?.length ? (
                detalles.map((detalle) => {
                    const detalleKey =
                        detalle.id ??
                        detalle.producto?.id ??
                        `${detalle.producto?.nombre ?? 'producto'}-${detalle.cantidad ?? 0}-${detalle.precioUnitario ?? 0}`;

                    return (
                        <View
                            key={String(detalleKey)}
...
function AccionesPedido({
    ...
    confirmarPago,
    cambiarEstado,
}: AccionesPedidoProps, Readonly) {
    const mostrarPreparando =
        estado === 'pendiente' && !pagoPendiente;

    const mostrarListo = estado === 'preparando';

    const mostrarEntregado = estado === 'listo';
            ...
            <View style={styles.accionesGrid}>
                {pagoPendiente && (
                    ...
                    >
                        ...
                        <ThemedText style={styles.btnAccionText}>
                            Confirmar Pago
                        </ThemedText>
                    </Pressable>
                )}
...
export default function AdminPedidoDetalleScreen() {
    ...
    const estadoInfo = getEstadoBadge(
    ...
    return (
        ...
        >
            ...
            <View style={styles.card}>
                ...
                <ThemedText style={styles.clienteInfo}>
                    {pedido.usuario?.email}
                </ThemedText>

                {pedido.usuario?.cedula ? (
                    <ThemedText style={styles.clienteInfo}>
                        Cédula: {pedido.usuario.cedula}
                    </ThemedText>
                ) : null}
            </View>

...
const CatalogoPage = () => {
   ...
   * y que se encuentre dentro de un rango seguro.
   */
  ...
  const loadCategorias = useCallback(async () => {
    try {
      ...
      const categoriasRecibidas = Array.isArray(res.data?.categorias)
        ? res.data.categorias
        : [];

      const categoriasValidas = categoriasRecibidas.filter((cat) =>
        isValidId(cat.id)
      );
  ...
  const loadSubcategorias = useCallback(async (catId) => {
    ...
    /*
     * El ID proviene de datos recibidos del servidor,
     * por lo que se valida antes de utilizarlo.
     */
    if (!isValidId(catId)) {
      setSubcategorias([]);
      return;
    ...
    try {
      const res = await catalogoService.getSubcategoriasPorCategoria(
        categoriaId
      );

      const subcategoriasRecibidas = Array.isArray(
        res.data?.subcategorias
      )
        ? res.data.subcategorias
        : [];

      setSubcategorias(subcategoriasRecibidas);
    } catch {
      setSubcategorias([]);
    }
  }, [isValidId]);
  ...
  const handleAddToCart = useCallback(async (producto) => {
    try {
      ...
      setMensaje({
        tipo: 'danger',
        texto: err.message || 'Error al agregar al carrito'
      });
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {mensaje.texto && (
          <div
            className={`mk-alert mk-alert-${mensaje.tipo}`}
